...
export default function ProgressPage() {
  ...
  const getProgress = () => {
    ...
    const total = session.scan_results?.length || 0;
  ...
  const handleStartScan = (positionId) => {
    navigate(createPageUrl("Scanner", `sessionId=${session.id}&positionId=${positionId}`));
  };
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <Tabs value={view} onValueChange={setView} className="mb-6">
        ...
        <TabsContent value="grid" className="space-y-6">
          <GantryView session={session} onStartScan={handleStartScan} />
        </TabsContent>

        <TabsContent value="list" className="space-y-6">
          <ListView session={session} onStartScan={handleStartScan} />
        </TabsContent>
      </Tabs>
...
function GantryView({ session, onStartScan }) {
  const mockPositions = [
        ...
        <div className="space-y-8 sm:space-y-12">
            ...
            <div className="flex space-x-2 sm:space-x-4 md:space-x-6 min-w-max px-2">
              {mockPositions.slice(0, 5).map((position) => (
                <GantryPosition
                  ...
                  onClick={() => onStartScan(position.id)}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-center">
            <GantryPosition
              ...
              onClick={() => onStartScan(mockPositions[5].id)}
            />
          </div>
        </div>
      </CardContent>
...
function GantryPosition({ position, status, onClick }) {
  ...
  return (
    <div className="flex flex-col items-center space-y-2 flex-shrink-0">
      <button
        onClick={onClick}
        className={`relative w-12 h-8 sm:w-14 sm:h-9 md:w-16 md:h-10 rounded-md border-2 transition-all duration-200 hover:scale-105 active:scale-95 ${getStatusColor()}`}
      >
...
function ListView({ session, onStartScan }) {
  const mockPositions = [
    { id: "p1", label: "Top-1", row: 0, column: 0 },
    { id: "p2", label: "Top-2", row: 0, column: 1 },
    { id: "p3", label: "Top-3", row: 0, column: 2 },
