...
export default function Export() {
  ...
  const handleCopyToClipboard = async (formatType = 'csv') => {
    ...
    try {
      const data = generateExportData(formatType);
  ...
  const handleEmailExport = async () => {
    ...
    try {
      ...
      if (error.message?.includes("outside the app") || error.message?.includes("404")) {
        alert("Email can only be sent to users within this app. Please use the copy or download options instead to share the data externally.");
      } else {
        alert("Failed to send email. Please try downloading or copying the data instead.");
      }
    } finally {
